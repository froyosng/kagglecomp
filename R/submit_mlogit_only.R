...
pred_ordered <- pred_mlogit_te[match(test_map$chid, rownames(pred_mlogit_te)), ]
pred_ordered <- pred_ordered[match(test$No, test_map$No), ]
pred_ordered <- pred_ordered / rowSums(pred_ordered)

submission <- data.frame(No = test$No, Ch1 = pred_ordered[,1], Ch2 = pred_ordered[,2],
                          Ch3 = pred_ordered[,3], Ch4 = pred_ordered[,4])
...
write.csv(submission, "submission_mlogit_m8trpg_only.csv", row.names = FALSE)
cat("Wrote submission_mlogit_m8trpg_only.csv\n")
