...
tr$log_incomea <- log1p(tr$incomea)
va$log_incomea <- log1p(va$incomea)

ctr <- sapply(tr[scaler_vars], mean, na.rm = TRUE)
scl <- sapply(tr[scaler_vars], sd, na.rm = TRUE)
scl[scl == 0] <- 1
ctr_log_income <- mean(tr$log_incomea, na.rm = TRUE)
scl_log_income <- sd(tr$log_incomea, na.rm = TRUE)

make_features <- function(df, ctr, scl, ctr_log_income, scl_log_income) {
  df <- as.data.frame(df)
  ...
  z_log_income <- (df$log_incomea - ctr_log_income) / scl_log_income
...
tr_feat <- make_features(tr, ctr, scl, ctr_log_income, scl_log_income)
va_feat <- make_features(va, ctr, scl, ctr_log_income, scl_log_income)
...
cat("Validation log loss (log-income spec):", round(ll, 6), "\n")
cat("(reference: confirmed base with raw linear incomea, single split = 1.159681)\n")
print(summary(mod)$CoefTable[grep("P_income|In_income", rownames(summary(mod)$CoefTable), value = TRUE), c("Estimate", "Pr(>|z|)")])
