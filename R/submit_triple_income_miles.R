## Submission candidate: m8trpg + Price x z(income) x z(mileage), blended
## 80/20 with the original xgboost (same recipe as ensemble_v11, only the
## mlogit component changes). CV-tested in R/codex_triple_interactions.R:
## mlogit alone 1.146782 vs 1.147021 baseline; fixed 80/20 blend 1.144599 vs
## ensemble_v11's 1.145094 (+0.000495 gain). Bootstrap 95% CI on that gain
## crosses zero ([-0.000689, 0.001660]) -- NOT CV-confirmed as a real
## improvement, being submitted for empirical validation against the public
## leaderboard, not because it cleared the project's submission bar.
...
make_features <- function(df, ctr, scl) {
  ...
  # New term: Price x standardized income x standardized mileage.
...
cat("P_income_miles coefficient (full-data fit):\n")
print(coef(mod_full)["P_income_miles"])
...
write.csv(submission, "submission_triple_income_miles.csv", row.names = FALSE)
cat("Wrote submission_triple_income_miles.csv\n")
cat("CV expectation: 1.144599 (fixed 80/20 blend); ensemble_v11 CV was 1.145094\n")
