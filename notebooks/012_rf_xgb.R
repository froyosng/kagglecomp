...
for (i in 1:nrow(xgb_grid)) {
  ...
  # FIXED: Extracted best iteration and score robustly
  best_iter <- which.min(cv_res$evaluation_log$test_mlogloss_mean)
  best_score <- min(cv_res$evaluation_log$test_mlogloss_mean)
...
for (f in 1:5) {
  ...
  oof_xgb[val_idx, ] <- matrix(predict(xgb_model, dval), ncol = length(label_map), byrow = TRUE)
}
...
final_xgb_probs <- matrix(predict(final_xgb, xgb.DMatrix(data = X_test_mm)),
                          ncol = length(label_map), byrow = TRUE,
                          dimnames = list(NULL, label_map))
...
write_csv(submission_012, "../results/submission_012_rf_xgb_blend.csv")
print("submission_012_rf_xgb_blend.csv has been generated!")